...
\mode<presentation> {
  ...
  \logo{\pgfputat{\pgfxy(0,5)}{\pgfbox[right,base]{}}}
  ...
  \begin{frame}
    ...
    \only<+>{\begin{block}{Embedded Model Predictive Control}
      \begin{center}
        ...
        \movie[externalviewer,autostart]{
    ...
    \only<+>{\begin{block}{Embedded Model Predictive Control}
      \begin{center}
        \movie[externalviewer,autostart,showcontrols,start=10s,duration=10s]{
  ...
  \begin{frame}
    ...
    \begin{itemize}
      ...
      {\color{red} \item not Scalable! Bottlenecks everywhere
                ...
                \item available sensors and peripheries are limited}
    \end{itemize}

    \begin{block}{Publications}
      \cite{baca2016embedded}, \cite{spurny2016complex}, \cite{saska2016auro}, \cite{chudoba2016exploration}
    \end{block}
  ...
  \begin{frame}
    ...
    \begin{block}{Realistic simulations of UAV grasping a brick}
      \begin{center}
        \movie[externalviewer,autostart,loop]{
  ...
  \begin{frame}
    ...
    \begin{block}{Real world UAV grasping a brick}
      \begin{center}
        \movie[externalviewer,autostart,loop]{
          \includegraphics[width=0.9\textwidth]{videos/brick_grasping_desert_thumbnail.jpg}
  ...
  \begin{frame}
    \frametitle{Vim -- modal, modular, modern text editor}
  ...
  %%{ Seminar assignemnt #1 -- Leader-Follower
  ...
  \begin{frame}
    \frametitle{System setup for assignment \#1}

    \begin{block}{Leader drone}
      \begin{itemize}
        \item out of your control
        \item follows a pre-planned trajectory
        \item equipped with UV LED blinkers under each rotor
        \item will make increasingly aggressive manoeuvres during the competition
      \end{itemize}
    \end{block}

    \begin{block}{Follower drone}
      \begin{itemize}
        \item equipped with two wide-angle UV-only cameras
        \item estimates leader relative position from camera stream only
        \item you will provide the control commands
      \end{itemize}
    \end{block}

    \begin{block}{Your task}
      \begin{itemize}
          \item periodically provide control commands to the follower
          \item keep the leader in sight for as long as possible
      \end{itemize}
    \end{block}
  ...
  %%{ Rules of competiton
  \begin{frame}
    \frametitle{Rules of competition}

    \begin{block}{Scoring}
      One point awarded for every 10 ms of active tracking
      \begin{itemize}
          \item counting starts once leader begins trajectory tracking
          \item end when leader reaches trajectory end or the task is terminated
      \end{itemize}
    \end{block}

    \begin{block}{Automatic task termination}
      \begin{itemize}
        \item leader not seen by follower cameras for $T_{max}$
        \item invalid control command provided $N_{err}$-times in a row
        \item no control command provided within $T_{control}$
      \end{itemize}
    \end{block}
  ...
  \begin{frame}
    \frametitle{Improving the default solution}

    \begin{block}{Default solution}
      Sets the desired position for the follower relative to the detected position of the leader 
    \end{block}

    \begin{block}{Potential improvements}
      \begin{itemize}
        \item experiment with the desired relative offset and control command rate
        \item use an estimator to estimate the leader's velocity (estimator provided)
        \item use the estimated velocity to design a better position reference
        \item use the estimated velocity to design a desired trajectory
        \item experiment with an option to control the follower with desired velocity directly
      \end{itemize}
      A more in-depth look into the system during the practicals and seminars. Additional tips and tricks will be provided throughout the week.
    \end{block}
  ...
  %%{ Video
  \begin{frame}
    \frametitle{Video of the default leader and follower approach}

    \vspace{-1.5em}

    \begin{center}
      \movie[externalviewer,autostart,loop]{
        \includegraphics[width=1.0\textwidth]{videos/uvdar_leader_follower_simulation_thumbnail.jpg}
      }{videos/uvdar_leader_follower_simulation.mp4}\\
    \end{center}

  \end{frame}
  %%}

  %%{ Seminar assignemnt #2-- MTSPN

  \begin{frame}
    \frametitle{Seminar assignment \#2 -- MTSPN}

    \begin{columns}[c]

      \column{0.45\textwidth} % Left column and width
      \begin{block}{A sample map of points}
        \includegraphics[width=1.0\textwidth]{fig/mtspn_task/tsp_points.png}
      \end{block}
  ...
  %%{ Video
  \begin{frame}
    \frametitle{Video from simulation}
    \begin{center}
      \vspace{-1.5em}
      \movie[externalviewer,autostart,loop]{
        \includegraphics[width=1.0\textwidth]{videos/mtspn_simulation_thumbnail.jpg}
      }{videos/mtspn_simulation.mp4}\\
    \end{center}
